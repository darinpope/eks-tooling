...
RUN cd /usr/local/bin && \
    ...
    /tmp/aws/install && \
    curl --silent --location "https://github.com/weaveworks/eksctl/releases/download/0.38.0/eksctl_Linux_amd64.tar.gz" | tar xz -C /tmp && \
    mv /tmp/eksctl /usr/local/bin && \
    curl -o kubectl "https://amazon-eks.s3.us-west-2.amazonaws.com/1.19.6/2021-01-05/bin/linux/amd64/kubectl" && \
    chmod +x /usr/local/bin/kubectl && \
    curl --silent --location "https://get.helm.sh/helm-v3.5.2-linux-amd64.tar.gz" | tar xz -C /tmp && \
